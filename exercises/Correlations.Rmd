...
What does this matrix mean? This is a rate matrix that states there is equal rate from 0-1 and 1-0.
...
What do these results mean? These news rates were calculated from hidden rates within the binary dataset in combination with our selected equal rate parameter. This suggests that the rates are still equal.
...
And these? This rate matrix suggests that you are twice as likely to go from state 1 to 2 than from 2 to 1. 
...
which model is better? If I had to choose it would be the lower AIC value - or pp.er but the AIC values are only 2 integers apart so it is unclear which is 'better'.
...
```{r, eval=TRUE}
print(rate.mat.maker(rate.cat=1, hrm=FALSE, ntraits=2, nstates=2, model="ARD"))
rate.mat.JEB<-rate.par.drop(rate.mat.ard.4state, drop.par=c(4, 7, 10))
print(rate.mat.JEB)

print(rayDISC(primates$tree, fourstate.data, ntraits=1, rate.mat= rate.mat.JEB, node.states="marginal",
              model="ARD"))

#seems unrealistic & bad AIC value- doubled from the last one
```
...
print(rayDISC(phy=primates$tree, data=fourstate.data, ntraits=1, rate.mat= rate.mat.JEB, node.states="marginal", root.p="yang", model="ARD"))
```
```{r, eval=TRUE}

print(rayDISC(phy=primates$tree, data=fourstate.data, ntraits=1, rate.mat= rate.mat.JEB, node.states="marginal", root.p="NULL", model="ARD"))
```

```{r, eval=TRUE}

print(rayDISC(phy=primates$tree, data=fourstate.data, ntraits=1, rate.mat= rate.mat.JEB, node.states="marginal", root.p="maddfitz", model="ARD"))
```
**Create and use a model to see if transitions from 00 go to 11 only via 01.**
```{r,eval=TRUE}
rate.mat.JEB2<-rate.par.drop(rate.mat.ard.4state, drop.par=c(2,7))
print(rate.mat.JEB2)

print(rayDISC(primates$tree, fourstate.data, ntraits=1, rate.mat= rate.mat.JEB2, node.states="marginal",
              model="ARD"))
```
```{r, eval=TRUE}
rate.mat.JEB3<-rate.par.drop(rate.mat.ard.4state, drop.par=c(1,4))
print(rate.mat.JEB3)

print(rayDISC(primates$tree, fourstate.data, ntraits=1, rate.mat= rate.mat.JEB3, node.states="marginal",
              model="ARD"))
```
```{r, eval=TRUE}
print(rayDISC(primates$tree, fourstate.data, ntraits=1, rate.mat= rate.mat.ard.4state, node.states="marginal",
              model="ARD"))
#Check original AIC value = 110.15; only going through 01 = 106; only going through 10 = 122; only going through 01 improves the AIC value slightly. 
```
##Route 2
